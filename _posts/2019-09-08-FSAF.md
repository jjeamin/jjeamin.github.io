...
FSAF의 목표는 각 instance가 네트워크를 최적화하기 위해 최상위 수준의 기능을 자유롭게 선택할 수 있도록 하는 것이므로 모듈에서 **기능 선택을 제한할 anchor box가 없어야 한다는 것** 이다.
...
$b_{e}^{l}$ 내부의 각 픽셀 위치 $(i,j)$에 대해서 `projected box` $b_{p}^{l}$ 를 4차원 벡터
$$

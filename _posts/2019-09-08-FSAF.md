...
예를 들어, 크기가 `50x50 pixel`인 자동차 instance와 크기가 `60x60 pixel`인 유사한 자동차 instance는 두 개의 서로 다른 level에 할당 될 수 있는 반면, 또 다른 `40x40 pixel` 자동차 instance는 동일한 level에 할당 될 수 있다.
...
$$v\epsilon _{i}$$,$$\epsilon _{e}$$ : 일정한 scale factor(각각 0.2 , 0.5)
...
- 두번째, `effective box`를 제외한  `ignoring box`는 회색 영역으로 표현된 영역이다.
...
`anchor-free branch`의 설계를 통해서 임의의 pyramid level $$P^l$$의 feature를 사용해서 각 instance를 학습 할 수 있다. instance box 대신 instance content를 기반으로 최적의 $$P^l$$을 선택한다.
...
- 2 $$L^{I}_{FL}(l),L^{I}_{IOU}(l)$$는 방정식을 사용하여 모든 `anchor-free branch`를 계산한다.

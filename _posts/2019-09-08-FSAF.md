...
추론하는 동안 예측 된 상자를 decoding 하는 것이 간단하다. 각 픽셀의 위치 $$i,j$$에서 예측된 offset 이
...
왼쪽 상단과 오른쪽 상단은 각각 $$(i-S_(\hat{o}_{t_{i,j}}^{l}),j-S_(\hat{o}_{l_{i,j}}^{l}))$$ 과 $$(i + S_(\hat{o}_{b_{i,j}}^{l}),j + S_(\hat{o}_{r_{i,j}}^{l}))$$

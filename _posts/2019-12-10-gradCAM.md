...
$$w_k^c = Z \cdot \frac{\partial Y^c}{\partial A^k}$$

모든 픽셀 $$\left (i, j \right )$$ 에 대해서 전부 합하면

$$\sum_i \sum_j w_k^c = \sum_i \sum_j Z \cdot \frac{\partial Y^c}{\partial A^k}$$

$$Z, w_k^c$$ 는 $$\left (i, j \right )$$ 에 의존하지 않으므로

$$Zw_k^c = Z \sum_i \sum_j \frac{\partial Y^c}{\partial A^k}$$

$$Z$$ 는 feature map의 픽셀의 수다. 그러므로

$$w_k^c = \sum_i \sum_j \frac{\partial Y^c}{\partial A^k}$$
